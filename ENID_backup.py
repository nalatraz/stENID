...
def TNS_get_class(object_id, object_metadata):
    print('Querying TNS for', object_id,'name and class verification...') 
    
    search_obj          = [("ra", str(object_metadata.iloc[1,1])), ("dec", str(object_metadata.iloc[1,2])), ("radius", "5"), ("units", "arcsec"), 
                           ("objname", ""), ("objname_exact_match", 0), ("internal_name", ""), 
                           ("internal_name_exact_match", 0), ("", ""), ("public_timestamp", "")]
    
    pause_needed = True
    
    while pause_needed == True:
        
        tns_search_response = search(search_obj)

        reset = get_reset_time(tns_search_response)

        if (tns_search_response.status_code == 200):
                if reset != None:
                    # Sleeping for reset + 1 sec
                    print("\033[1m Timeout Required : \033[0m Sleeping for " + '\033[1m' + str(reset + 1) + '\033[0m' + " seconds...")
                    time.sleep(reset + 1)
                
                else:
                    pause_needed = False
                
    tns_search_response_json = json.loads(tns_search_response.text)
    
    if not tns_search_response_json['data']['reply']:
        tns_name = 'Unknown'
        tns_class = 'None'
        tns_class_id = ''
        
    else:
        tns_name = tns_search_response_json['data']['reply'][0]['objname']

        get_obj = [("objname", tns_name), ("objid", ""), ("photometry", "1"), ("spectra", "0")]

        tns_get_response = get(get_obj)
        tns_get_response_json = json.loads(tns_get_response.text)

        tns_class = tns_get_response_json['data']['reply']['object_type']['name']
        tns_class_id = tns_get_response_json['data']['reply']['object_type']['id']

    print('Object', str(tns_name), 'TNS Class :', '\033[1m' + str(tns_class) + '\033[0m', '\n')
    
    return tns_class, tns_class_id

def dataset_gen(classes):
    
    lightcures = pd.DataFrame()
    ...
    for transient_class in classes:
        ...
        for object_name in transient_objects:
            ...
            if object_data is None:
                ...
                TNS_class, TNS_label = TNS_get_class(object_name, object_metadata)
